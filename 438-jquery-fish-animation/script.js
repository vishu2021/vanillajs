var RENDERER = {
  ...
  render: function () {
    ...
    // Define gradient color stops (you can add more stops as needed)
    gradient.addColorStop(0, "lightblue"); // Start color (e.g., light blue at the top)
    gradient.addColorStop(1, "blue"); // End color (e.g., darker blue at the bottom)
    // Set the gradient as the fill style for the water
...
FISH.prototype = {
  ...
  render: function (context) {
    ...
    // Set the fill style to golden (e.g., '#FFD700')
...
FISH.prototype.render = function (context) {
  ...
  context.rotate(Math.PI + Math.atan2(this.vy, this.vx));
  context.scale(1, this.direction ? 1 : -1);
  // Set the fill style to golden (e.g., '#FFD700') for the fish body
  context.fillStyle = "goldenrod";
  // Draw the fish body
  context.beginPath();
  context.moveTo(-30, 0);
  ...
  context.beginPath();
  context.arc(-15, -5, 3, 0, 2 * Math.PI); // Left eye
  context.arc(-15, 5, 3, 0, 2 * Math.PI); // Right eye
  context.fill();
  context.save();
  context.translate(40, 0);
  context.scale(0.9 + 0.2 * Math.sin(this.theta), 1);
