$(document).ready(function () {
      ...
      if (!isNaN(tokenList[i])) {
        ...
        while (opStack[opStack.length - 1] !== "bracket-left") {
          ...
          if (operator.numOperands === 1)
            ...
            valStack.push(
              applyOperator(operator, [valStack.pop(), valStack.pop()])
            );
        }
        opStack.pop();
      } else {
        while (
          opStack.length > 0 &&
          hasPrecedence(opStack[opStack.length - 1], tokenList[i])
        ) {
          var operator = getOperator(opStack.pop());
          if (operator.numOperands === 1)
            valStack.push(applyOperator(operator, [valStack.pop()]));
  ...
  function output(out, expression, tokens) {
    ...
    for (var i = calcHistory.length - 1; i >= 0; i--) {
      $("#calc-history-box").append(
        "<p style='color: #B0B0B0; ' class='calc-history-eq' id='eq" +
          i +
          "'>" +
          calcHistory[i].expression +
          "</p><p style='text-align: right; margin-top: -10px;'>= " +
          calcHistory[i].out +
          "</p>"
      );
    }
  ...
  function addToken(token) {
    if (isNaN(token)) {
      ...
      if (!isNaN(tokenList[tokenList.length - 1])) {
        ...
        if (
          !isNaN(token) &&
          (tokenList[tokenList.length - 1] === "bracket-right" ||
  ...
  $(document).on("click", ".calc-history-eq", function (event) {
    var tokens =
      calcHistory[parseInt($(event.target).attr("id").substring(2))].tokens;
